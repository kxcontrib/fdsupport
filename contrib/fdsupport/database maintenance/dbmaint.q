/ 2006.06.28 - update os functions 
/ kdb+ partitioned database maintenance - pdurkan@firstderivatives.com
if[.z.w;'`only.from.console]

/ * private
\d .os
WIN:.z.o in`w32`w64
rpth:{p:$[10h=type x;x;string x];if[WIN;p[where"/"=p]:"\\"];(":"=first p)_ p} / relative path
apth:{$[$[(first p:rpth x)in"/\\";1b;$[WIN;p like"[a-zA-Z]:*";0b]];p;$[WIN;system"cd";first system"pwd"],"/\\"[WIN],p]} / absolute path
cpy:{system 0N!$[WIN;"copy /v /z ";"cp "],rpth[x]," ",r:apth y;r}
del:{system 0N!$[WIN;"del /q ";"rm -r "],r:apth x;r}
mov:{system 0N!$[WIN;"move ";"mv "],rpth[x]," ",r:apth y;r}
...
enum:{[tabledir;col;val]
    if[not 11=abs type val;:val];
    .[p;();,;u@:iasc u@:where not(u:distinct enlist val)in v:$[type key p:(`)sv tabledir,col;get p;0#`]];col!(v,u)?val}
...
fix1table:{[tabledir;goodpartition;goodpartitioncols]
	if[count missing:goodpartitioncols except allcols tabledir;
		...
		{[gp;m]add1col[tabledir;m;0#get gp,m]}[goodpartition]each missing]}
...
rename1col:{[tabledir;oldname;newname]
	if[(oldname in ac)and not newname in ac:allcols tabledir;
		...
		.os.mov[(`)sv tabledir,oldname;newname];
...
addcol:{[dbdir;table;colname;defaultvalue] / addcol[`:/data/taq;`trade;`noo;0h]
	if[not validcolname colname;
		'(`)sv colname,`invalid.colname];
    add1col[;colname;enum[dbdir;colname;defaultvalue]]each allpaths[dbdir;table];}
...
something you may want to add to your copy of dbmaint(i dont think this functionality exists in that script)

needed it to change TAQ db so that i can add extra syms that have more than one letter exchanges.
/ function to change char column to symbol in historical database
/ dst|partition|table|field
c2e:{[d;p;t;f]r set sv[`;d,`sym]?`$enlist each get r:` sv .Q.par[d;p;t],f}
